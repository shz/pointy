...
be the only release series to exist).
...
Pointy is coded entirely in C# 2.0, and runs without a hitch on Mono.
	...
	writing, the fastest Python web server available) and node.js 0.1.27 (generally one
	of the fastest application servers period):

	ab -c 30 -n 10000
	node.js: ~2550 requests/second
	Tornado: ~1600 requests/second
	Pointy:  ~1800 requests/second
	(TODO - update this with results from the faster Pointy :D)
